/*  Although insertion can be done in parallel using one global lock. Here we are using array of locks for each bucket. Retrieving from hash table does not require lock. Retrieval is already running in parallel threads
    and we do not need lock for shared resource for retrieval. */

/*
   There is an overhead when we add spinlock in comparison to the version that uses mutex.
   This is because Spin lock is a mechanism in which the process needing a resource poll the lock on resource till it gets it.
   And Mutex is kind of a key to use any resources in the system. If you have mutex, you can use the resource, if
   you don’t wait till the mutex is released. Process goes to wait queue for that particular resource.
 
   Sample Output :

   ./parallel_mutex 10
    Inserted 100000 keys in 0.034382 seconds
    Retrieved 100000/100000 keys in 4.400079 seconds


   ./parallel_spin 10
    Inserted  100000 keys in 0.048977 seconds
    Retrieved 100000/100000 keys in 25.195034 seconds
 */
...
void * put_phase(void *arg) {
    ...
    for (key = tid ; key < NUM_KEYS; key += num_threads) {
       pthread_spin_lock(&spinlock); //acquire spin lock
        insert(keys[key], tid);
       pthread_spin_unlock(&spinlock); //release spin lock
    }
